  ...
  useFrame(({ clock }) => {
    ...
    if (t - lastSpawn.current > 1.4) {
      ...
      const mat  = new THREE.MeshBasicMaterial({
        color: "#E8512A", transparent: true, opacity: 0.7,
    ...
    <div className="absolute inset-0 pointer-events-none z-10 font-mono p-4 md:p-6 overflow-hidden">
      ...
      <div className="flex justify-between items-start w-full gap-4 relative z-20">
        ...
        <div className="text-[7px] md:text-[9px] tracking-[0.22em] uppercase leading-[1.8] text-white/30 max-w-[48%]">
          <div className={cn("font-bold", scanDone ? "text-[#E8512A]/70" : "text-[#E8512A]")}>
            {scanDone ? "● Scan Complete" : `● Scanning  ${pct}%`}
          </div>
          <div className="opacity-80">Visible: {visible.toLocaleString()} Pts</div>
          <div className="hidden sm:block opacity-60">System: LiDAR Terrestrial</div>
        </div>
        ...
        <div className="text-[7px] md:text-[9px] tracking-[0.22em] uppercase leading-[1.8] text-white/25 text-right max-w-[48%]">
          ...
          <div className="hidden sm:block opacity-60">Sensor: ROAM-LIDAR-01</div>
        </div>
      </div>
          ...
          {scanDone && (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="block w-1.5 h-1.5 rounded-full bg-[#0D2B1F]" />Ground
              </div>
              <div className="flex items-center gap-2">
                <span className="block w-1.5 h-1.5 rounded-full bg-[#2A5C3F]" />Moss
              </div>
              <div className="flex items-center gap-2">
                <span className="block w-1.5 h-1.5 rounded-full bg-[#8A7B5C]" />Rock
              </div>
              <div className="flex items-center gap-2">
                <span className="block w-1.5 h-1.5 rounded-full bg-[#E8512A]" />Coral
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {[
        ...
        <div
          key={cls}
          className={cn("absolute w-4 h-4 border-[#E8512A]/20 transition-opacity duration-700", scanDone ? "opacity-10" : "opacity-30", cls)}
        />
      ))}
    </div>
  );
...
export default function PointCloudCanvas() {
  ...
  return (
    <div
      ...
      onPointerDown={() => { if (scanDone) setUserControl(true); }}
    >
      <Canvas
