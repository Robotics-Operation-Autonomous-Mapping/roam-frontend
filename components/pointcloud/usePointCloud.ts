...
function heightToColor(t: number): THREE.Color {
  ...
  if (t < 0.55) return C_MID.clone().lerp(C_ROCK, (t - 0.25) / 0.3);
  if (t < 0.8) return C_ROCK.clone().lerp(C_HIGH, (t - 0.55) / 0.25);
  return C_HIGH.clone().lerp(C_CANOPY, (t - 0.8) / 0.2);
}
