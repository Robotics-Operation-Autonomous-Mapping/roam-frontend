...
export const ComputeMesh: React.FC<Props> = ({ mats }) => (
  <group>
    ...
    {Array.from({ length: 12 }).map((_, i) => (
      <mesh key={i} position={[-0.27 + i * 0.05, 0.32, 0]} material={mats.greyMat}>
    ...
    {/* Status LEDs */}
    {[0, 1, 2].map(i => (
      <mesh key={i} position={[-0.32 + i * 0.06, 0.12, 0.46]} material={mats.ledMat}>
    ...
    {Array.from({ length: 5 }).map((_, i) => (
      <mesh key={i} position={[-0.1 + i * 0.05, 0, -0.46]} material={mats.darkMat}>
