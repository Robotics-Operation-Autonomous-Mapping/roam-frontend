...
const PARTS: RigPart[] = [
  {
    ...
    details:
      "Main compute housing featuring heavy-duty ribbed heat sinks and ruggedized M12 I/O ports.",
    position: [0, 0, 0],
  },
  {
    id: "lidar_top",
    ...
    details:
      "High-frequency spinning optical array with 32-channel laser emitters and internal mirror mechanics.",
    position: [0, 1.2, 0],
  },
  {
    id: "rgb_left",
    ...
    details:
      "Left eye of the stereo vision system with multi-element glass optics and an integrated sun hood.",
  ...
  },
  {
    ...
    spec: "Wide stereo channel",
    details:
      "Right eye of the stereo vision system with multi-element glass optics and an integrated sun hood.",
    position: [1.4, 0.3, 0.6],
  },
  {
    id: "ir",
    ...
    details:
      "Thermal package with a specialized germanium lens, designed to see through dust and darkness.",
    position: [0, -0.2, 1.4],
  },
  {
    id: "gps",
    ...
    details:
      "Low-profile RTK-capable GPS puck on a vibration-damped stem for absolute global positioning.",
    position: [0, 1.0, -1.2],
  },
  {
    id: "imu",
    label: "Precision IMU",
    spec: "9-DOF Inertial Unit",
    details:
      "High-frequency gyroscope, accelerometer, and magnetometer package for dead reckoning and stabilization.",
    position: [0, -0.65, 0.5],
  },
  {
    id: "esc",
    label: "Motor Controllers",
    spec: "4x 50A ESC Array",
    details:
      "High-power motor drivers featuring oversized electrolytic capacitors and thick aluminum heat dissipation blocks.",
    position: [-1.1, -0.8, -0.5],
  },
  {
    id: "pdb",
    label: "Power Distribution",
    spec: "High-Current PDB",
    details:
      "Routes battery power safely to logic and drive systems with integrated buck converters and heavy-duty fuses.",
    position: [1.1, -0.8, -0.5],
  },
  {
    id: "telemetry",
    label: "Telemetry Radio",
    spec: "900MHz Long-Range",
    details:
      "Maintains a high-bandwidth connection for manual override, RTK corrections, and remote diagnostics.",
...
const RigMesh: React.FC<{
  ...
  const bodyColor =
    part.id === "lidar_top" ? "#E8512A" : isActive ? "#5B667A" : "#3F4756";
  const emissiveColor =
    part.id === "lidar_top" ? "#E8512A" : isActive ? "#5B667A" : "#000000";
    ...
    >
      {part.id === "core" && (
        <group>
          ...
          {/* IO Ports Front */}
          <mesh position={[-0.3, 0, 0.46]} material={accentMat}>
      ...
      {part.id === "lidar_top" && (
        <group>
          {/* Base mount */}
          <mesh position={[0, -0.15, 0]} material={accentMat}>
          ...
          <mesh material={lensMat}>
            <cylinderGeometry args={[0.225, 0.225, 0.12, 32]} />
          </mesh>
          {/* Top cap */}
      ...
      {(part.id === "rgb_left" || part.id === "rgb_right") && (
        <group>
          ...
          {/* Backplate */}
          <mesh position={[0, 0, -0.26]} material={accentMat}>
          ...
          {/* Sun hood */}
          <mesh position={[0, 0.18, 0.4]} material={bodyMat}>
      ...
      {part.id === "ir" && (
        <group>
          ...
          {/* Lens base */}
          <mesh position={[0, 0, 0.22]} material={accentMat}>
            <boxGeometry args={[0.25, 0.25, 0.1]} />
          </mesh>
          {/* IR Lens */}
          <mesh position={[0, 0, 0.28]} material={irLensMat}>
      ...
      {part.id === "imu" && (
        <group>
          ...
          {/* Screws */}
          <mesh position={[-0.2, -0.06, 0.1]} material={heatsinkMat}>
          ...
          <mesh position={[0, 0.11, 0]} material={accentMat}>
      ...
      {part.id === "esc" && (
        <group>
          {/* PCB Base */}
          <mesh position={[0, -0.05, 0]} material={pcbMat}>
            <boxGeometry args={[0.6, 0.04, 0.4]} />
          </mesh>
          {/* Heatsink */}
          <mesh position={[-0.1, 0.05, 0]} material={heatsinkMat}>
          ...
          {/* Gold terminals */}
          {[-0.15, 0, 0.15].map((z, i) => (
            <mesh key={`t-${i}`} position={[-0.3, 0, z]} material={goldMat}>
      ...
      {part.id === "pdb" && (
        <group>
          ...
          {/* Big Terminals */}
          <mesh position={[-0.25, 0.06, 0.15]} material={goldMat}>
          ...
          {/* Fuses */}
          {[-0.1, 0, 0.1].map((z, i) => (
            <mesh key={i} position={[0.25, 0.04, z]} material={irLensMat}>
              <boxGeometry args={[0.08, 0.06, 0.04]} />
            </mesh>
          ))}
          {/* Small ICs */}
          <mesh position={[0.1, 0.03, -0.15]} material={accentMat}>
      ...
      {part.id === "telemetry" && (
        <group>
          ...
          <group position={[0.18, 0, 0.1]} rotation={[0, 0, -Math.PI / 6]}>
            <mesh position={[0, 0.3, 0]} material={accentMat}>
...
export const ExplodedSensorRig: React.FC = () => {
  ...
  const activeDetails =
    activePart?.details ??
    "Tap or click a component to inspect subsystem details.";
  ...
  return (
    <div className="w-full mt-12 border border-border bg-surface/50">
      <div className="px-6 pt-5 pb-3 flex items-center justify-between border-b border-border">
        ...
        <span className="font-mono text-xs tracking-widest text-cream/80 uppercase">
          {activeLabel}
        </span>
      </div>
